...
class StreamingMonitor:
    def __init__(self, window_size=60):
        ...
        self.schema_versions = deque(maxlen=window_size)
        self.is_running = False
        self.lock = threading.Lock()
    
    def record_message(self, processing_time_ms, schema_version=None):
    ...
    def _report_metrics(self):
        ...
        with self.lock:
            ...
            # Báo cáo các phiên bản schema nếu có thông tin
            if self.schema_versions:
                schema_counts = {}
                for version in self.schema_versions:
                    if version in schema_counts:
                        schema_counts[version] += 1
                    else:
                        schema_counts[version] = 1
                print(f"Phân bố phiên bản schema: {schema_counts}")
...
if __name__ == "__main__":
    ...
    try:
        for i in range(100):
            ...
            monitor.record_message(proc_time, schema_version=1 if i < 50 else 2)
